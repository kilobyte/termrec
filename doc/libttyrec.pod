...
This function opens a stream designated by the I<url>.  If the given url ends in ".gz", ".xz" or ".bz2"), the
stream is assumed to be compressed and gets passed through the appropriate [un]packer.  If the I<fd> is not -1,
...
=item B<recorder        ttyrec_w_open(int >I<fd>B<, const char *>I<format>B<, const char *>I<filename>B<, const struct timespec *>I<ts>B<);>
...
=item B<int     ttyrec_w_write(recorder >I<r>B<, const struct timespec *tm, const char *>I<data>B<, int >I<len>B<);>
...
=item B<int     ttyrec_r_play(int >I<fd>B<, const char *>I<format>B<, const char *>I<filename>B<,    void (*>I<synch_init_wait>B<)(const struct timespec *ts, void *>I<arg>B<),    void (*>I<synch_wait>B<)(const struct timespec *>I<delay>B<, void *>I<arg>B<),    void (*>I<synch_print>B<)(const char *>I<data>B<, int >I<len>B<, void *>I<arg>B<),    void *>I<arg>B<);>
...
=item B<ttyrec_frame    ttyrec_seek(ttyrec >I<tr>B<, const struct timespec *>I<t>B<, tty *>I<vt>B<);>
...
=item B<void            ttyrec_add_frame(ttyrec >I<tr>B<, const struct timespec *>I<delay>B<, const char *>I<data>B<, int >I<len>B<);>

Creates a new frame and appends it to ttyrec I<tr>.

=item B<int             ttyrec_save(ttyrec >I<tr>B<, int >I<fd>B<, const char *>I<format>B<, const char *>I<filename>B<, const struct timespec *>I<selstart>B<, const struct timespec *>I<selend>B<);>
...
=head2 B<struct timespec> arithmetics:

A handful of macros for operating on timespec values:
